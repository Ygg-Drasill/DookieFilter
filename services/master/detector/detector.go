...
func (w *Worker) swap(parity bool, p map[string]types.PlayerPosition) {
    ...
    if len(p) > 4 {
        fmt.Println("More than 2 players detected")
    }
    ...
    for _, prev := range pf {
        for _, curr := range cf {
            if prev.player.PlayerId != curr.player.PlayerId && positionProximity(prev, curr) {
                swapPlayers(p, curr, findPair(cf, curr))
                swapped = true
                break
            }
        }
...
func findPair(players []swapPlayer, exclude swapPlayer) swapPlayer {
    for _, p := range players {
        if p.key != exclude.key {
            return p
        }
    }
    return exclude
