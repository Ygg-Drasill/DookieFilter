...
// cT is the coordinates change threshold for proximity,
// currently set to 2.5%
var cT = 2.5

func positionProximity(p1, p2 swapPlayer) bool {
    x1 := p1.player.Position.X
    x2 := p2.player.Position.X
    y1 := p1.player.Position.Y
    y2 := p2.player.Position.Y
    ptX := math.Abs(x1 - x2)
    ptY := math.Abs(y1 - y2)
    pbX := (x1 + x2) / 2
    pbY := (y1 + y2) / 2
    pdX := (ptX / pbX) * 100
    pdY := (ptY / pbY) * 100
    fmt.Println("pdX", pdX, "pdY", pdY)
    return pdX < cT && pdY < cT
}
